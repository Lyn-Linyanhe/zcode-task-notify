...
- **微信简单指令**：在机器人单聊里发「帮助 / 状态 / 静默 / 静默 30 / 恢复 / 最近」即可查开关、定时静默、回看最近推送（指令只认主人，见已知坑 10；未启用手机批准时改 `state.json`）
...
之后在单聊里发「帮助 / 状态 / 静默 / 静默 30 / 恢复 / 最近」就能遥控通知系统。**这里只做轻量指令**（查状态、开关通知、回看推送）；真正要 agent 干活的任务，请打开手机远程控制页面看它具体怎么执行。
